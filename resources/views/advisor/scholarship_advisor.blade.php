...
@section('content')
   ...
   @php
   $allStatusesComplete = 1;
   if (isset($budget)) {
       $editStatus = $budget->edit_status;
       
   } else {
       $editStatus = null; // Handle the case where $edit is not set
   }
@endphp

@foreach ($user_info_Advisor as $info)
   @if ($info->cv_status != 1 || $info->motivation_status != 1 || $info->passport_status != 1 || $info->transcript_status != 1 || $editStatus != 0)
       @php
           $allStatusesComplete = 0;
       @endphp
   @endif
@endforeach
    @if (count($user_info_Advisor) > 0)
        <div class="alert alert-warning" role="alert" style="text-align: center; width: 50%; margin: 0 auto;"><i
                class="fa-solid fa-triangle-exclamation"></i> สามารถเลือกนักศึกษาได้
            1 
            คนเท่านั้นหากเลือกแล้วจะไม่สามารถทำการแก้ไขได้  
        </div>
                    ...
                    @foreach ($user_info_Advisor as $info)
                        <tr class="no-border-bottom">
                            <td class="info-container" style="text-align: left;">
                                {{ ($user_info_Advisor->currentPage() - 1) * $user_info_Advisor->perPage() + $loop->iteration }}
                            </td>
                            ...
                            <td>
                                ...
                                @elseif(
                                    ...
                                    &nbsp;&nbsp;&nbsp;<a style="color: orange;"><i
                                            class="fa-solid fa-hourglass-start"></i>&nbsp;Waiting for interview</a>
                            </td>
                            <td>

                                @if ($allStatusesComplete == 1)
                        ...
                        @else
                            <a style="color: orange;">&nbsp;&nbsp;&nbsp;<i
                                    class="fa-solid fa-hourglass-start"></i>&nbsp;Haven't checked</a>
                    @endif
                    </td>
                        ...
                        <div class="modal-dialog modal-dialog-centered modal-lg">
                            <div class="modal-content" style="width: 500px">
                                <h5 class="modal-title h1form" id="exampleModalLabel" style="text-align: center">
                                    <br>
                                    Applicants required file
                                </h5>
                                <hr class="custom-hr">
                                <div class="modal-body">
                                    <div style="text-align: center;">
                                        <form
                                            action="{{ route('Advisor.update', $info->info_id) }} "
                                            method="POST">
                                            @csrf
...
@else
    ...
    <div class="alert alert-warning" role="alert" style="text-align: center; width: 50%; margin: 0 auto;"><i
            class="fa-solid fa-triangle-exclamation"></i> ไม่มีนักศึกษาสมัคร
    </div>
    @endif
    <!-- if program == 1 show user data but program == 0 blank card -->
@endsection
